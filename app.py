...
def _build_schedule_backups(df_existing: pd.DataFrame) -> tuple[bytes, bytes]:
    """Return (csv_bytes, xlsx_bytes) for the current schedule."""
    csv_bytes = df_existing.to_csv(index=False).encode("utf-8")
...
with col_save:
    # Save button for the data editor (add row section)
    if st.button("💾 Save Changes", key="manual_save_full_edit", use_container_width=True, type="primary"):
...
with col_save:
    # Save button for the data editor (add row section)
    if st.button("💾 Save Changes", key="manual_save_full_add", use_container_width=True, type="primary"):
        st.session_state.manual_save_triggered = True
...
# Keep In Time and Out Time as time objects for proper display
display_all["In Time"] = display_all["In Time"].apply(lambda v: v if isinstance(v, time_type) else None)
display_all["Out Time"] = display_all["Out Time